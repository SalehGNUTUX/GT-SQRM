// ═══════════════════════════════════════════════════════
//  GT-SQRM v1.0 — GnuTux Short Quran Reels Maker
...
function _applyPreviewMute() {
  ...
  // 1. WebAudio gainNode (المشغّل الحالي)
...
function _applyExportMute() {
  ...
  // 1. كل gainNodes في التصدير
  if (S.exportSources && S.audioCtx) {
    S.exportSources.forEach(({ gain }) => {
      if (gain) gain.gain.setTargetAtTime(muted ? 0 : vol, S.audioCtx.currentTime, 0.03);
    });
  }
  // 2. gainNode المشغّل الحالي
  if (S.recGainNode && S.audioCtx) {
...
function cleanupExportMute() {
  ...
  const vol = gv("rec-vol") / 100;
  if (S.recGainNode && S.audioCtx) {
    try { S.recGainNode.gain.setTargetAtTime(vol, S.audioCtx.currentTime, 0.05); } catch(_) {}
  }
  // exportSources قد تكون فارغة بعد التصدير — لكن نُعيد الضبط احتياطاً
  if (S.exportSources) {
    S.exportSources.forEach(({ gain }) => {
      if (gain && S.audioCtx) {
        try { gain.gain.setTargetAtTime(vol, S.audioCtx.currentTime, 0.05); } catch(_) {}
      }
    });
  }
  // ── إعادة ضبط عناصر الصوت ───────────────────────────
  if (S.bgAudioEl)  S.bgAudioEl.muted  = false;
  if (S.recAudioEl) S.recAudioEl.muted = false;

  // ── إعادة ضبط واجهة الزر ────────────────────────────
  const btn  = $("export-mute-btn");
  const hint = $("export-mute-hint");
  if (btn)  { btn.textContent = "🔊 كتم الصوت"; btn.classList.remove("btn-muted"); }
  if (hint) hint.textContent  = "الصوت مُشغَّل — اضغط لكتمه";
  // مسح مرجع recGainNode القديم لضمان إنشاء node جديدة عند التشغيل التالي
  S.recGainNode = null;
}
...
async function startExport(type) {
  ...
  initExportMuteState(); // كتم الصوت إن كان الإعداد مفعّلاً
...
async function applyDownloadedMedia(filePath, type, srcUrl) {
  ...
  if (type === "video") {
    ...
    toast("✅ الفيديو جاهز كخلفية", "success");
  } else if (type === "audio") {
